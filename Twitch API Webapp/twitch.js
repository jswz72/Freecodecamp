$(document).ready(function(){
  ...
  //data gained from pseudo api-call
  var twitchData = [
  {
    "stream": {
      "mature": false,
  ...
  //var clientID = "?client_id=ldee7ukveef0jq0zikbveiunanqo6u&callback=?";
  console.log(twitchData.length); //how many users
  
  //iterate through users and print relevant info
  for(var i = 0; i < twitchData.length; i++){
  
  if(twitchData[i].stream === null){
    $("#streamers").append("<a href = '" + twitchData[i].url+ "' target = 'blank'><li>" + twitchData[i].display_name + " offline </li></a>" );
  }
    else{
      $("#streamers").append("<div class = 'online-info'><a href = '" + twitchData[i].stream.url + "' target = 'blank'><li>" + twitchData[i].stream.display_name + " online </li></a>" );
      $("#streamers").append("<li>" + twitchData[i].stream.game + "</li>");
      $("#streamers").append("<li>" + twitchData[i].stream.status + "</li></div>")
    }
  }
